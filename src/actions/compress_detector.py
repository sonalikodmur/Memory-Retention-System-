...
def run_compression_pipeline():
    ...
    scored["is_compressed_before"] = False
    ...
    for _, row in candidates.iterrows():
        ...
        results.append({
            ...
            "compressed_word_count": word_count(compressed),
            "retention_score": row["retention_score"],
            "action_taken": "COMPRESS",
            "compressed_on": NOW.strftime("%Y-%m-%d"),
    ...
    if len(compressed_df) > 0:
        avg_original = compressed_df["original_word_count"].mean()
        avg_compressed = compressed_df["compressed_word_count"].mean()
        reduction_pct = (1 - avg_compressed / avg_original) * 100 if avg_original > 0 else 0
        print(f"\nStep 5: Summary statistics")
        print(f"  Total compressed memories: {len(compressed_df)}")
